...
export default function UserLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const user = await login({ ...form, portal: 'user' })
      if (user.role === 'peso' || user.role === 'clcdo' || user.role === 'admin') {
        throw new Error('This portal is for users only. Please use the staff login page.')
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">User Portal • Sign in as a jobseeker or employer</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="form-group">
              ...
              <div className="relative">
                ...
                <input type="email" className="input pl-10" placeholder="you@example.com" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} required autoComplete="email" />
              </div>
            </div>
            <div className="form-group">
          ...
          <p className="text-center text-sm text-gray-500 mt-2">
            Staff member? <Link to="/login/staff" className="font-semibold text-blue-700 hover:underline">Use staff login</Link>
          </p>
        </div>
      </div>
    </div>
